...
import { projects, CardVariant } from "../data";
          ...
          {projects.map((project, index) => {
            ...
            return (
              ...
              >
                ...
                {!isDark && (
                  <div
                    className={`overflow-hidden rounded-t-3xl flex-shrink-0 bg-slate-200 ${isLarge ? "h-52 sm:h-64" : "h-44"}`}
                  >
                    <img
                      src={project.image}
                      alt={project.title}
                ...
                <div className="relative flex flex-col flex-1 p-6 justify-between">
                  <div>
                    <p
                      className={`text-[10px] font-bold tracking-[0.3em] uppercase mb-3 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                    >
                      {String(index + 1).padStart(2, "0")}
                    </p>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <h3
                        className={`font-bold leading-snug ${isLarge ? "text-xl sm:text-2xl" : "text-lg"} ${isDark ? "text-white" : "text-slate-900"}`}
                      >
                        {project.title}
                      </h3>
                      <a
                    ...
                    <p
                      className={`text-sm leading-relaxed ${isLarge ? "line-clamp-3" : "line-clamp-2"} ${isDark ? "text-slate-400" : "text-slate-500"}`}
                    >
                      {project.description}
                    </p>
                  </div>
