...
import { Code2, Cloud, Database, Server, ArrowRight } from "lucide-react";
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        {theme === "minimal" ? (
            ...
            <div className="mt-16 lg:mt-0 grid grid-cols-2 gap-x-12 gap-y-12 w-full lg:w-auto lg:border-l border-gray-200 lg:pl-16">
              {skillsList.map((skill, i) => (
                ...
                >
                  ...
                  <span className="text-xs font-bold tracking-[0.2em] uppercase text-black mb-1">
                    {skill.label}
                  </span>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="grid grid-cols-2 lg:grid-cols-4 lg:grid-rows-2 gap-6"
          >
            <div className="col-span-2 lg:col-span-2 lg:row-span-2 flex flex-col justify-center text-left p-8 sm:p-10 md:p-14 bg-white rounded-[2.5rem] shadow-sm border border-slate-200">
            ...
            {skillsList.map((skill, i) => (
              <motion.div
                key={i}
                whileHover={{ scale: 1.02 }}
                className="col-span-1 flex flex-col items-start justify-between p-5 sm:p-7 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 min-h-[150px] lg:min-h-[170px]"
              >
                <div className="p-3 sm:p-4 bg-blue-50 rounded-2xl mb-4 sm:mb-6">
                  <skill.icon className="w-6 h-6 sm:w-8 sm:h-8 text-blue-600" />
                </div>
                <div>
                  <span className="block text-base sm:text-lg font-bold text-slate-900 mb-1">
                    {skill.label}
                  </span>
