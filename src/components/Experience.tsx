...
const iconSlugs: Record<
  string,
  string | { monochrome: string; color: string }
> = {
  Ruby: "ruby",
  ...
  Java: {
    monochrome:
      "https://img.icons8.com/ios-filled/500/java-coffee-cup-logo--v1.png",
...
export default function Experience({ theme }: { theme: ThemeType }) {
  ...
  const getIconSource = (skill: string, useColor: boolean = false) => {
    ...
    if (!mappedValue)
      return `https://cdn.simpleicons.org/javascript${colorSuffix}`; // Fallback
  ...
  return (
    <section
      className={`py-20 ${theme === "minimal" ? "bg-white" : "bg-slate-50"}`}
      id="experience"
    >
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="max-w-4xl mx-auto">
          <div
            className={`space-y-4 text-lg leading-relaxed ${theme === "minimal" ? "text-gray-700" : "text-slate-600"}`}
          >
            <p>
              I'm a computer science professional with 7 years of experience in
              building robust, scalable and fault-tollerant SaaS based
              applications with high throughput and low latency by designing
              efficient system and database architecture adhering to design
              principles and implementing necessary design patterns ensuring
              both performance and reliability. I have worked across multiple
              product-based as well as service-based organizations from
              different domains including apparel, home automation, finance,
              interior design etc. My primary tech stack has been Ruby on Rails
              but I also have experience with LAMP and MERN stack. I have worked
              with multiple SQL databases like MySQL and PostgreSQL with noSQL
              databases like MongoDB and Redis. I also have experience with
              cloud infrustucture like AWS S3, EC2, Lambda, DynamoDB etc. My
              CI/CD and devops skills include shell scripts, Github actions,
              nginx, Docker, Kubernetes, Airflow and Ansible.
            </p>
          </div>
        </div>
